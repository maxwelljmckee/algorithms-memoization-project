...
function minChange(coins, amount, memo = {count: 0}) {
  let lastIndex = coins.length - 1;
  let currentCoin = coins[lastIndex];
  let remainder = amount % coins[lastIndex];
  ...
  memo['count'] += Math.floor(amount / currentCoin);
  minChange(coins.slice(0, lastIndex), remainder, memo);
  return memo['count'];
}
minChange([1, 4, 5], 8)
